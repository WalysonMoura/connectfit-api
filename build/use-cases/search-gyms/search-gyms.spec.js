...
function getDistaceBetweenCoordinates(from, to) {
  ...
  let dist2 = Math.sin(fromRadian) * Math.sin(toRadian) + Math.cos(fromRadian) * Math.cos(toRadian) * Math.cos(radTheta);
  if (dist2 > 1) {
    dist2 = 1;
  }
...
// src/use-cases/search-gyms/search-gyms.spec.ts
var gymRepository;
var sut;
describe("", () => {
  beforeEach(async () => {
  ...
  it("", async () => {
    ...
    globalExpect(gyms).toHaveLength(1);
    globalExpect(gyms).toEqual([
      globalExpect.objectContaining({ title: "JavaScript Gym" })
    ]);
  });
  it("", async () => {
    for (let i2 = 1; i2 <= 22; i2++) {
      await gymRepository.create({
        title: `JavaScript Gym ${i2}`,
        description: null,
        phone: null,
        latitude: -27.2092052,
        longitude: -49.6401091
    ...
    console.log(gyms);
    globalExpect(gyms).toEqual([
      globalExpect.objectContaining({ title: "JavaScript Gym 21" }),
      globalExpect.objectContaining({ title: "JavaScript Gym 22" })
    ]);
  });
});
