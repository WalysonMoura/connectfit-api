...
// src/use-cases/validate-check-ins/validate-check-ins.spec.ts
...
describe("", () => {
  ...
  it("", async () => {
    ...
    globalExpect(checkIn.validated_at).toEqual(globalExpect.any(Date));
    globalExpect(checkInRepository.checkIns[0].validated_at).toEqual(
      globalExpect.any(Date)
    );
  });
  it("", async () => {
    ...
    const twentyOneMinutesInMs = 1e3 * 60 * 21;
